 ...
 */
export function FAB({ icon = 'add', label = '', variant = 'standard', onclick = null, className = '' }) {
  // Resolve classes from active theme
  const fabClass = cn(
    resolveClasses('fab', { variant }),
    'm3-fab',
  ...
  const template = ({ icon, label, variant }) => `
    <button class="${fabClass}" data-ref="btn" data-rnx-ignore="true">
